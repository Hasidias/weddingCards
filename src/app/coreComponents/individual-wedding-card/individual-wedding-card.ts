...
updatePrice() {
  if (this.selecteOptions.paperType !== '' && this.selecteOptions.color !== '') {
    ...
    if (index !== -1) {
      ...
      this.price = unitPrice * (this.order.quantity ? this.order.quantity : 1);
...
async CalculatePriceAfterDiscount() {
  ...
  for (let [index,promo] of this.cardData.promotions!.entries()) {
    new Promise<void>((resolve) => {
      ...
      const range = this.findInRange(this.order.quantity, rangesWithMinMax);
      if (range) {
...
orderNow(type:string) {
  if (this.isloggedInUser == null) {
    console.log('user not logged in');
    this.router.navigate(['/signin'], { queryParams: { returnUrl: this.activatedRoute.snapshot.url }}); 
    return
  };
  if (this.isEmailVerified == false ) {
    console.log('user email not verified');
    this.router.navigate(['/signin'], { queryParams: { returnUrl: this.activatedRoute.snapshot.url, mode:'forgotEmail' }});
    return
  };
  ...
  switch(this.cardData.category) {
    case 'weddingCards':
      this.order.attributes = ['color', 'paper']
      break;
  }
  ...
  if (this.giftBox) {
    ...
    this.order.giftBox.designImageURL = this.cardData.variances[this.selectedVarianceIndex].giftBoxM[this.selectedGiftBoxDesignIndex].designImageURL;
  };
  ...
  this.order.isGiftBoxIncluded = this.giftBox;
  this.order.orderDate = new Date().getTime();
  this.order.customer = {
  ...
  this.order.category = this.cardData.category;
  this.order.id = `${this.cardData.category.substring(0,3)}-${new Date().getTime()}`;
  this.cardData.variances.map(variance => {
    if (variance.color == this.selecteOptions.color ) {
      this.order.imgURL = variance.imageURL;
    }
  });
  this.order.name = this.cardData.name;

  if (type == 'regular') {
    this.router.navigate(['/checkout'], { state: {orders: [this.order]}});
  }
  if (type == 'cart') {
    console.log(this.order)
    if (this.uid) {
      this.dbService.addToCart(this.order, this.uid)
    } 
  }

  if (type == 'customize') {
    if (this.uid) {
      this.dbService.submitCustomizedOrder(this.order, this.uid)
    }
  }
  // console.log('final order :', this.order);
  //   this.dbService.placeOrder({
  //     ...this.order
  //   }).then(() => {
  //     this.order = new WeddingCardOrder();
  //     this.selecteOptions = {paperType: '', color: ''};
  //     this.price = 0;
  //     this.giftBox = false;
  //     this.standardGiftBoxSelected = true;
  //     this.selectedGiftBoxDesignIndex = 0;
  //     this.selectedGiftBoxMPricingIndex = 0;
  //     this.TotalAfterPromotions = 0;
  //     this.promotions = [];
  //     this.eligiblePromotion = {type:'', discount: 0};
  //     this.giftBoxTotal = 0;
  //     this.customizedBoxDimenstions = {};
  //     this.cdr.detectChanges();
  //   });
  }
}